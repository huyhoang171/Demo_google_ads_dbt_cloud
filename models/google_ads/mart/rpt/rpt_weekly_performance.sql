{{
    config(
        materialized='table',
        description='Weekly performance trends aggregated across all campaigns'
    )
}}
...
final as (
    select
        date_trunc(stat_date, week(monday)) as week_start_date,
        date_add(date_trunc(stat_date, week(monday)), interval 6 day) as week_end_date,
        format_date('%Y-W%W', stat_date) as year_week,
    ...
    group by 
        date_trunc(stat_date, week(monday)),
        date_add(date_trunc(stat_date, week(monday)), interval 6 day),
        format_date('%Y-W%W', stat_date)
)
